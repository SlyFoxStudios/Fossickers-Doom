...
{
    public ObsidianWallTile(final int id) {
        super(id);
    }
    ...
    public void render(final Screen screen, final Level level, final int x, final int y) {
        ...
        final int col13 = Color.get(59, 59, 59, 59);
        final int col14 = Color.get(0, 159, 59, 59);
        final int col15 = Color.get(0, 59, 59, 59);
    ...
    public boolean interact(final Level level, final int xt, final int yt, final Player player, final Item item, final int attackDir) {
        if (item instanceof ToolItem) {
            ...
            if (ModeMenu.creative) {
                this.hurt(level, xt, yt, 999);
                return true;
            }
            if (tool.type == ToolType.pickaxe && tool.level > 2 && AirWizard.beaten && player.payStamina(4 - tool.level)) {
