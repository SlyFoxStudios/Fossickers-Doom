...
public class StoneBrickTile extends Tile
{
    ...
    public StoneBrickTile(final int id, final boolean nearspawner) {
        super(id);
        this.nearspawner = false;
        this.nearspawner = nearspawner;
    }
    
    @Override
    public void render(final Screen screen, final Level level, final int x, final int y) {
        final int col0 = Color.get(222, 222, 333, 333);
        final int col2 = Color.get(333, 333, 444, 444);
        final int col3 = Color.get(222, 222, 333, 333);
        final int col4 = Color.get(111, 111, 222, 222);
        final int col5 = Color.get(333, 333, 444, 444);
    ...
    public boolean interact(final Level level, final int xt, final int yt, final Player player, final Item item, final int attackDir) {
        if (item instanceof ToolItem) {
            ...
            if (tool.type == ToolType.pickaxe && player.payStamina(4 - tool.level)) {
                ...
                level.add(new ItemEntity(new ResourceItem(Resource.sbrick), xt * 16 + this.random.nextInt(10) + 3, yt * 16 + this.random.nextInt(10) + 3));
