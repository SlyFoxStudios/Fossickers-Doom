...
public class Screen
{
    ...
    public void render(int xp, int yp, final int tile, final int colors, final int bits, final int scale) {
        xp -= this.xOffset;
        yp -= this.yOffset;
        final boolean mirrorX = (bits & 0x1) > 0;
        final boolean mirrorY = (bits & 0x2) > 0;
        final int xTile = tile % 32;
        final int yTile = tile / 32;
        final int toffs = xTile * scale + yTile * scale * this.sheet.width;
                    ...
                    for (int x = 0; x < scale; ++x) {
                        if (x + xp >= 0) {
                            if (x + xp < this.w) {
                                ...
                                final int col = colors >> this.sheet.extrapixels[xs + ys * this.sheet.width + toffs] * 8 & 0xFF;
                                if (col < 255) {
                                    this.extrapixels[x + xp + (y + yp) * this.w] = col;
                                }
                            }
                        }
                    }
